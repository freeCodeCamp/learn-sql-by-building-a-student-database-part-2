...
In your script, add an `echo` at the bottom to print all the course names whose first letter is before `D` in the alphabet, as suggested by the previous line in the file. Use a similar pattern that you used earlier to get and print data from the database. Make sure to use double quotes where needed.
